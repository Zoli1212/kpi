...
async function seedKPI() {
  ...
  const systemMap = Object.fromEntries(systems.map(sy => [sy.name.trim().toLowerCase(), sy.id]));
  ...
  const kpis = [
    ...
    kpi.itemId && kpi.serviceId && kpi.companyId && kpi.systemId && kpi.userId
  );
...
seedKPI()
  .then(() => {
    console.log("🎯 KPI createMany seed kész");
    prisma.$disconnect();
  })
  .catch((e) => {
